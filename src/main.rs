...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    // block until at least one byte read, with given timeout
    uart.set_read_mode(0, UARD_READ_TIMEOUT)?;
    let mut buffer = [0_u8; TX_BUFFER_LEN];
    ...
    loop {
        ...
        // do a blocking receive on the channel as we initiate the rs485 comms
        match tcp_to_485_rx.recv() {
            Ok(message) => {
                // send the packet out of the uart
                println!("Sending down uart");
                rts_pin.set_high();
                message.encode(&mut buffer).unwrap();
                uart.write(&buffer)?; // default non-blocking
                ...
                */
                ...
                if message.message_type != Some(MessageType::Poll) {
                    // if poll message then go to listening mode on uart,
                    // otherwise wait for another message to transmit
                    continue;
                }
            }
            //Err(RecvError::Empty) => {}
        ...
        //
        // * loop trying to read a byte from the buffer
        // * if some timeout reached then give up and go back to listening for
        //   a message to transmit
        while uart.read(&mut buffer)? > 0 {
            //loop {
